...
export namespace Eval {
  ...
  export function evaluation(source, context, prefix = '') {
  ...
  export function exec(source, context) {
    try {
      ...
      const toReturn = new Function(...params, `
        const value = ${ source};
        return value == undefined ? '' : Number.isNaN(value) ? 0 : value;
      `)(...paramsValues);
      if (toReturn === undefined) { return evaluation(source, context); }
      return toReturn;
    } catch (e) { return evaluation(source, context); }
  }
}
